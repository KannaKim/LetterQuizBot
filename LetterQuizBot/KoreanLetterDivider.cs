...
using System.Text;
//this code was modified from http://www.hoons.net/Board/asptip/Content/20889
...
namespace LetterQuizBot     
{
    ...
    /// 한글 분석 클래스
    /// </summary>
    public sealed class KoreanLetterDivider
    {
        /// <summary>
        /// 초성 리스트
        /// </summary>
        ...
        public static HANGUL_INFO DevideJaso(char hanChar)
        {
            ...
            if ((temp < m_UniCodeHangulBase) || (temp > m_UniCodeHangulLast)) // 캐릭터가 한글이 아닐 경우 처리
            {
                hi.isHangul = "NH";
                hi.originalChar = hanChar;
                hi.chars = null;
                if (HTable_ChoSung.Contains(hanChar) || HTable_JongSung.Contains(hanChar))
                    hi.chars =new char[] {hanChar};
            }
            else
            {
